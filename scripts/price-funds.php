...
// Close the DB connection through the sleep period
...
include("/var/www/html/".$_SESSION['base_url']."scripts/dbConnect.php");

// Get the memberID and last login for everyone who has logged in within the past X days (value passed and defined above) or who is flagged as a Master, Teacher, or Student (or an employee/admin)
if ($duration > 0){
	$query = "
		...
		AND (last_login > :cutoff_date OR f.promote = 1 OR f.teacher = 1 OR f.student = 1 OR f.composite = 1 OR f.staff = 1 OR f.admin = 1
		GROUP BY member_id
	";
}else{  // Do 'em all
//	$query = "
...
//		usleep(500000); // 1/2 second
	...
	// Reconnect to database after sleeping
//	include("/var/www/html/".$_SESSION['base_url']."secure/dbconnect.php");
	include("/var/www/html/".$_SESSION['base_url']."scripts/dbConnect.php");
...
// Skip Bruce Pile (loosegun) so we don;t wipe out the manual entries!
//if ($members['member_id'] != "686"){
//      continue;
//}
...
//	continue;
//}
	// If we have submitted enough API queries to fill up the available ports, take a breather and let the API free some up
	if($requests % 400 == 0){
		sleep(5);
	}
...
//die($preparedQuery);
		...
		if ($firstDate <= $_SESSION['max_date']){
			...
			$endDate = substr($_SESSION['max_date'], 0, 4)."-".substr($_SESSION['max_date'], 4, 2)."-".substr($_SESSION['max_date'], 6, 2);
 			...
 			// Go get 'em
 			include("/var/www/html/".$_SESSION['base_url']."web/includes/get-fund-pricing-batch.php");
...
// Reactive this query to see if it adds stability to the members' stratification
