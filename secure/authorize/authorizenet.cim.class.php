 ...
 *
 * Version 1.4 March 16, 2015 (Marketocracy Custom by JSS)
 ...
 * Changelog:
 *
 * -- March 16, 2015 - Version 1.4 (JSS)
 * Modified the parseResults function pretty heavily to add a lot more specific value assignments to the returned results for later use.
 * parseResults() also needed modifications to properly parse new array style results for some parameters that used to be single value.
 * Fixed a few other bugs here and there.
 *
...
class AuthNetCim {
	...
	var $version = '1.4'; // the code revision number for this class
	var $params = array();
	var $LineItems = array();
	var $success = false;
	var $error = true;
...
//		$subdomain = ($this->test_mode) ? 'apitest' : 'api';
		$subdomain = ($this->test_mode) ? 'apitest' : 'api2'; // Akamai change
...
//die($this->response);  // Testing
	...
	{
		// Parse response and determine if transaction was approved or declined (if applicable)
		if (stripos($this->response,'approved') !== false) {
			$this->approval = 'Approved';
		}else if (stripos($this->response,'denied') !== false || stripos($this->response,'declined') !== false) {
			$this->approval = 'Declined';
		}else{
			$this->approval = '';
		}

		$this->merchantCustomerId = $this->substring_between($this->response,'<merchantCustomerId>','</merchantCustomerId>');
		$this->email = $this->substring_between($this->response,'<email>','</email>');
		...
		$this->phoneNumber = $this->substring_between($this->response,'<phoneNumber>','</phoneNumber>');
		$this->cardNumber = $this->substring_between($this->response,'<cardNumber>','</cardNumber>');

		// Parse response and extract transaction ID (if applicable)
		$transChunk = $this->substring_between($this->response,'approved.,','XXXX');
		$aTransChunks = explode(",", $transChunk);
		$this->transID = $aTransChunks[2];

		// Parse response and extract authorization code (if applicable)
		$this->authCode = $this->substring_between($this->response,'approved.,',',Y');
		...
		// Grab customerPaymentProfileId - test if within an array, if not, then test for it by itself
		if (strpos($this->response,'<customerPaymentProfileIdList><numericString>') !== false) {
		...
		// Grab customerAddressId - test if within an array, if not, then test for it by itself
		if (strpos($this->response,'<customerShippingAddressIdList><numericString>') !== false) {
			$this->customerAddressId = $this->substring_between($this->response,'<customerShippingAddressIdList><numericString>','</numericString></customerShippingAddressIdList>');
		}else{
