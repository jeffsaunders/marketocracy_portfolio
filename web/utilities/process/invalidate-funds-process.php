...
if($process == "invalidate-funds"){
	//echo 'hello';
	
	$apiServer = $_REQUEST['api'];
	$showQueries = $_REQUEST['run'];
	...
	echo $apiServer.'|'.$showQueries;
	
	/*$query = "
		SELECT fi.*, mf.fund_id, mf.fund_symbol FROM `members_fund_invalidated` as fi 
		INNER JOIN `members_fund` as mf ON mf.fb_primarykey= CONCAT(\"X'\",fi.fb_primarykey,\"'\")
		WHERE fi.processed='0'
	";*/
	
	$query = "
		SELECT fi.*, mf.fund_id, mf.fund_symbol, m.username, mf.unix_date AS inception
		FROM `members_fund_invalidated` as fi 
		INNER JOIN `members_fund` as mf ON mf.fb_primarykey= CONCAT(\"X'\",fi.fb_primarykey,\"'\")
	...
	while($funds = $rsGetFunds->fetch(PDO::FETCH_ASSOC)){
		...
		$aFunds[$fundID] = array(
			...
			'username'		=> $username,
			'invalidate'	=> $funds['invalidate_date'],
			'inception'		=> $funds['inception'],
			'uid'			=> $uid
		);	
		
	}
	foreach($aFunds as $fundID=>$aFundInfo){
		$invalidate 	= $aFundInfo['invalidate'];
		$inception		= $aFundInfo['inception'];
		
		$year			= substr($invalidate, 0, 4);
		$month			= substr($invalidate, 4, 2);
		$day			= substr($invalidate, 6, 2);
	...
	switch($apiServer){
		case 'api1': $portStart = 52000;$portStop = 52099;$startPort = rand(52000,52099);break;
		case 'api2': $portStart = 52100;$portStop = 52499;$startPort = rand(52100,52499);break;
                case 'api3': $portStart = 52500;$portStop = 52699;$startPort = rand(52500,52699);break;
	}
	...
	foreach($aFunds as $fundID=>$aFundInfo){
		...
		$query = "
			UPDATE members_fund_invalidated	
			SET processed='1'
			WHERE uid=:uid
		";
		try{
...
if($process == 'file-upload'){
	...
	switch($apiServer){
		...
		case 'api2': $port = rand(52100, 52499);break;
