...
if($fundID == ''){
	...
	if($memberID == ''){
		...
		$aQuereies['select'][] = $preparedQuery;
...
foreach($aFunds as $key=>$fundID){
	...
	$aQuereies['update'][] = $preparedQuery;
	...
	foreach($aTrades as $stockSymbol=>$aTradeType){
		...
		switch($sector){
			...
			case 'Financials'						: $sectocCode = '7';break;
		...
		$aQuereies['delete'][] = $preparedQuery;
		...
		$aQuereies['insert'][] = $preparedQuery;
