...
if($process == 'submit-cart-order'){
	...
	$orderDescription	= $cartInfo['product_desc'];
	$itemID				= $cartInfo['product_id'];
	$itemName			= $cartInfo['product_name'];
	$unitPrice			= $cartInfo['sale_price'];
	$tax_amount			= $cartInfo['sales_tax'];
	$transaction_amount	= $cartInfo['total_price'];
	...
	if ($CIMresultCode == "Error"){
		...
		// Pop the trailing pipe off
		$discounts = substr($discounts, 0, -1);
		...
		try{
			...
			$aValues = array(
				...
				':bill_frequency'		=> $term,
				':next_bill_timestamp'	=> $renewalDate,
				':next_bill_amount'		=> $transaction_amount,
				':fund_id'				=> null,
		...
		try{
			...
			$aValues = array(
				':standard'	=> ($itemID == '2' ? '1' : '0'),
				':premium'	=> ($itemID == '3' ? '1' : '0'),
				':member_id'=> $_SESSION["member_id"]
			);
			$preparedQuery = str_replace(array_keys($aValues), array_values($aValues), $query); //Debug
			//$aErrors[] = $preparedQuery;
		...
		$_SESSION['flag_standard'] = ($itemID == '2' ? '1' : '0');
		$_SESSION['flag_premium'] = ($itemID == '3' ? '1' : '0');
			...
			function cartDbInsert($mLink,$aInsertValues){
				...
				try {
					...
					$aValues = array(
						...
						':renewal'		=> $aInsertValues['renewalDate'],
                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="cart-price">
                            <h4>$<?php echo $aCartItem['sale_price'];?></h4>
...
if($process == 'checkout-step'){
	...
	switch($step){
		#Cart
		case 0:
			$prevButton = '<button id="clear-cart" type="button" class="btn btn-default" onclick="cart(\'clear\',\'\',\'\',\'\');" style="float:left;" data-dismiss="modal">Clear Cart</button>';
...
if($process == 'checkout'){
    ...
    <div class="panel-group accordion" id="checkout_parent">
            ...
            <div class="panel-heading accordion-fix">
                <h4 class="panel-title">
                    <a class="accordion-toggle" onclick="checkoutStep('1');" data-toggle="collapse" data-parent="#checkout_parent" href="#checkout-summary" style="height:40px;<?php echo $barColor;?>">
                        ...
                        <span class="accordion-icons accordion-expand"></span><span style="float:right;margin-top:5px;margin-right:5px;"><?php echo $numItems;?> Items | Subtotal: <strong>$<?php echo $subTotal;?></strong></span><span style="float:right;display:inline;padding-right:10px;"> <?php echo $barButton;?></span>
                    </a>
                </h4>
            </div>
            <div id="checkout-summary" class="panel-collapse collapse">
        ...
        <div class="panel panel-default">
            ...
            <div id="checkout-discounts" class="panel-collapse in">
                <div class="panel-body">
                
                	<h4>Apply Discount Code</h4>
                    <p>Do you have a discount code? Enter it below and click apply. If you do not have a discount code conintue on to payment methods.</p>
...
print_r($aCart);
	...
	if($payMethodID != ''){
/*	This is defined in the cart and passed as $payMethodID
	if($payMethodID != 'xx'){
...
*/
		...
		//Calcualte TEXAS state/county sales tax (8.25%)
		...
		if (!isset($_SESSION['cart_id']) || empty($_SESSION['cart_id'])){
			...
			// Yup!  Create a cart_id that will eventually become the invoice number
			$_SESSION['cart_id'] = $_SESSION["member_id"]."-".time();
			...
			try{
				...
				$aValues = array(
					...
					':term'			=> $term,
					':renewal'		=> $renewalDate,
					':sale_price'	=> $subTotal - $discountTotal,
					':orig_price'	=> $subTotal,
					':sales_tax'	=> $salesTax,
			...
			// Existing cart, just update it.
			$query = "
				UPDATE ".$_SESSION['cart_table']."
				SET cart_id			= :cart_id,
					member_id		= :member_id,
