...
Supporting files:
	AJAX		- THIS DOCUMENT process/ajax/fund-trades-processes-new.php
	JAVASCRIPT	- includes/scripts/fund-trades-new.js.php
	HTML		- includes/pages/fund-trades-new.php
*/
//Start Session
session_start();
...
//Process Functions
function tradesCheck($mLink, $fundID, $symbol, $fundSymbol=false, $split=false){
    ...
    <table class="table table-bordered table-striped table-condensed flip-content volatility-table">
        ...
        <tbody style="text-align:right;">
            ...
            foreach($aBuys as $key=>$aBuyTrades){
				...
				?>
                <tr>
                	<td style="text-align:left;"><a href="#ticket-details" data-toggle="modal" onclick="loadTicketDetail('<?php echo $uid;?>','<?php echo $fundID;?>');"><?php echo $date;?></a></td>
    ...
    <div style="width:500px;margin:0px auto 10px auto;">
        <div style="float:left;width:150px;">
            ...
            <div class="input-group">
                ...
                <input type="submit"  class="btn btn-info" value="Quote" style="text-transform:uppercase;" />
                </span>
            </div>
            </form>
        </div>
            ...
            foreach($aTrades as $key=>$aTrade){
				...
				if($shares != '0'){
					...
					<tr>
						...
						<td><a href="#ticket-details" data-toggle="modal" class="btn btn-xs btn-default" onclick="loadTicketDetail('<?php echo $uid;?>','<?php echo $fundID;?>');">Details</a></td>
					</tr>
					<?php
					}
				}
        ...
        foreach($aFunds as $fundID=>$aFund){
            ...
            if($fundID != 'stockInfo'){
                ...
                if($aFund['cash'] != $aFund['adjustedCash']){
                    $showAdjustCash = ' <i class="icon-exclamation-sign" style="cursor:help;color:#ECBC29;" title="Adjusted Cash: $'.number_format($aFund['adjustedCash'], 2, '.',',').' - This is your adjusted cash value based on trades that are currently pending."></i>';
                    $showAdjustShares = '<i class="icon-exclamation-sign" style="cursor:help;color:#ECBC29;" title="Adjusted Max Shares: '.number_format($aFund['adjustBuyMaxShares'], 0, '.',',').' - This is your adjusted Max Share value based on trades that are currently pending."></i>';
                }else {
                    $showAdjustCash = '';
                    $showAdjustShares = '';
                }
                ...
                <tr>
                    ...
                    <td>
                        <?php echo number_format($aFund['totalShares'],0,'.',',');?>
                        <input type="hidden" name="sell-limit-shares-<?php echo $fundID;?>" value="<?php echo $aFund['totalShares'];?>"/>
                    </td>
                    <td>
...
if($process == '6') {
    ...
    switch($type){
        ...
        case 'sell':
            ...
            foreach($aTrades as $fundID=>$aTrade){
                ...
                if($shares > $sellLimit){
                    $aErrors[] = $aTrade['fundSymbol'].': You can not sell more shares than you own.';
                }
    ...
    if(empty($aErrors)){
        ...
        foreach($aTrades as $fundID=>$aTrade) {
            ...
            //Create query
            $query = 'create||' . $username . '|' . $fundSymbol . '|' . $fundID . '|' . $tradeType . '|' . $orderType . '|' . $symbol . '|' . $shares . '|' . $limitPrice . '|' . $reasons . '|' . $tradeDesc . '';
