...
function printRow($aTrade, $mLink, $update){
		...
		$currentPrice	= number_format($aTrade['currentPrice'],4,'.',',');
        ...
        <?php } ?>
			...
			<td class="cprice-col" <?php if(in_array('cprice-col',$aHideCols)){echo 'style="display:none;"';}?>>
				$<?php echo $currentPrice;?> <input type="hidden" value="<?php echo $aTrade['currentPrice'];?>" id="current-price-<?php echo $cnt;?>" name="current-price-<?php echo $cnt;?>" /><br />
...
if($process == "2"){
	...
	foreach($aSymbols as $key=>$symbol){
		...
		$aTrades[$symbol] = array(
			...
			'symbol'		=> $symbol,
			'currentPrice'	=> $foo['CurrentPrice']/*number_format(round($foo['CurrentPrice'], 2), 2, '.', '')*/,
			'change'		=> $foo['chang'],
			'exclude'		=> $exclude,
	...
	foreach($aTrades as $key => $aTrade){
		...
		if($currentPrice == "0" || $currentPrice == ""){
			...
			$event = 'Trade Error';
			$detail	= 'No pricing record was found for symbol: '.$symbol.' | Fund: '.get_funds($mLink, $fundID, 'fundSymbol');
			...
			echo '
				<tr>
					<td>'.$rowCnt.'</td>
					<td>'.$symbol.'</td>
					<td colspan="12">No pricing record was found for this symbol. We have been notified of this issue, but if you would like to submit a support ticket, please <a href="/?page=08-01-00-001&type=pp&subtype=trade_issue&ticketSubject=No%20Pricing%20Record%20for%20RUTH&issueFund='.$fundID.'" target="_blank">click here</a>. '.$error.' | '.$currentPrice.'</td>
				</tr>
			';	
		}else{
			//debug
