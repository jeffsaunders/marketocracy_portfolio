            ...
            <div class="response-body">
                ...
                <div class="response-actions">
					...
					if($cntAgree != 0){
						...
						?>
                    	<button type="button" class="btn btn-<?php echo $btnStatus;?> btn-xs" onClick="agreeDisagree('agree','<?php echo $reply['uid'];?>', '<?php echo $reply['member_id'];?>','<?php echo $reply['ticket_id'];?>')" title="<?php echo $btnTitle;?>"><?php echo $btnType; ?></button>
                    	<?php
					}else{
						?>
						<button type="button" class="btn btn-success btn-xs" onClick="agreeDisagree('agree','<?php echo $reply['uid'];?>', '<?php echo $reply['member_id'];?>','<?php echo $reply['ticket_id'];?>')">Agree</button>
						<?php
                    }
					// END AGREE BUTTON
					...
					if($cntDisagree != 0){
						...
						?>
                    	<button type="button" class="btn btn-<?php echo $btnStatus;?> btn-xs" onClick="agreeDisagree('disagree','<?php echo $reply['uid'];?>', '<?php echo $reply['member_id'];?>','<?php echo $reply['ticket_id'];?>')" title="<?php echo $btnTitle;?>"><?php echo $btnType;?></button>
                    	<?php
					}else{
						?>
						<button type="button" class="btn btn-info btn-xs" onClick="agreeDisagree('disagree','<?php echo $reply['uid'];?>', '<?php echo $reply['member_id'];?>','<?php echo $reply['ticket_id'];?>')"><?php if($cntDisagree != 0){echo "(".$cntDisagree.") ";}?> Disagree</button>
						<?php
                    }
					//END DISAGREE BUTTON
					
					?>
                    <button type="button" class="btn btn-warning btn-xs" onClick="showReply('<?php echo $reply['uid'];?>')" title="Click here to reply to soandso's response">Reply</button>
                </div><!--response-actions-->
                <div class="clear"></div><!--clear-->
                
                <div id="replyForm-<?php echo $reply['uid'];?>" style="display:none;">
                    <!-- BEGIN FORM-->
                    <form action="process/ajax/support-processes.php?process=12" method="post" class="reply-<?php echo $reply['uid'];?>" name="reply-<?php echo $reply['uid'];?>">
                        <div class="form-body">
                            <div id="replyUserError-<?php echo $reply['uid'];?>">
                            </div>
                                
                            <div class="form-group">
                                <textarea name="reply" id="reply-<?php echo $reply['uid'];?>" class="form-control" rows="4" style="resize:vertical;"></textarea>
                            </div>
                            <input type="hidden" name="ticket_member_id" value="<?php echo $reply['ticket_member_id']; ?>" />	
                            <input type="hidden" name="ticket_id" value="<?php echo $reply['ticket_id']; ?>"  />
                            <input type="hidden" name="uid" value="<?php echo $reply['uid'];?>" />
                                    
                            <input type="submit" onClick="postReply('<?php echo $reply['uid'];?>')" value="Reply" class="btn btn-warning btn-sm">
                            
                       <button type="button" onClick="hideReply('<?php echo $reply['uid'];?>')" class="btn btn-sm btn-info">Cancel</button>
                        </div><!--form-body-->
                    </form>
                    <!-- END FORM-->
                </div><!--reply-form-->
