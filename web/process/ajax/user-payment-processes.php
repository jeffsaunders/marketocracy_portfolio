...
if($process == 'new-payment-method'){
	...
	if($country == "xx"){
		$aErrors[] = 'Please select the country for the billing address.';	
	}
...
//$testMode = true;
////////////////////////////////////////////////// AIM is not working right on the live account.  I suspect a config error on our account,  For now just bypass the validation check.
////////////////////////////////////////////////// Really don't need it anyway as the CIM createPaymentProfile method includes such a check internally....
/*
		$amount = (rand(1, 10) / 10); // Random value between .01 and .10
...
//echo empty($aErrors);
//		if (empty($aErrors)){
		if (1==1){  // Always run until we figure out the AIM issue above

			// See if the member has a merchant account record.
			$query = "
...
//			$rowCount = $rsCustomer->fetchColumn();
			$customer = $rsCustomer->fetch(PDO::FETCH_ASSOC);
			$rowCount = $customer['count'];
//echo $rowCount;
//sleep(10);
			//$aErrors[] = $error;

			// If no results then they don't have a merchant record, create it.
//			if ($rsCustomer->rowCount() < 1){  //rowCount() is unreliable in PDO...
			if ($rowCount < 1){
				...
				// Create the account at Authorize
				...
				// Did it fail?
//				if (strtoupper($resultCode) != "OK" && $resultCode != ''){
				if (strtoupper($resultCode) != "OK"){

					// Add to error array
					$aErrors[] = "Transaction failed with the following error: ".$code." - ".$text." - ".$resultCode;
...
//$aErrors[] = "customerAddressId = ".$customerAddressId;
				...
				// Grab their profile id.
				// Couldn't just do this above (where the COUNT() was performed) because of a PDO/MySQL rowCount() bug - results are unreliable!
				$query = "
...
//			$authMemberID = $_SESSION['member_id'];
				...
				// Create the payment profile record at Authorize
...
//				if (strtoupper($resultCode) != "OK" && $resultCode != ''){
				if (strtoupper($resultCode) != "OK"){
					...
					try{
						...
						$aValues = array(
							':member_id'	=> $_SESSION["member_id"],
							':task'			=> "authorizeCreatePaymentProfile",
							':result_code'	=> $resultCode,
							':response_code'=> $code,
							':response_text'=> $text
						);
...
if($process == 'load-states'){
	...
	switch($countryCode){
		...
		default:
			...
			$stateLabel		= 'State/Province/Region<span class="required"></span>';
			$stateDropDown	= '';
			$stateTextField = 'State/Province/Region (Leave Blank if not applicable)';
...
if($process == 'load-invoice-modal'){
	...
	$invoiceID = $_REQUEST['uid'];
	
	$transInfo = transactionDetails($mLink,$invoiceID,'invoiceNumber','html');
		//echo '<pre>';
		//print_r($transInfo);
		//echo '</pre>';
	...
	<div class="modal-header">
       <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
       <h4 class="modal-title">Invoice</span></h4>
    </div>
