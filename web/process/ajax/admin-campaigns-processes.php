...
switch($process){
	...
	case 'load-emails':
		...
		$query = "
			...
			WHERE camp_type='standard_campaign' AND deleted<>'1' OR camp_type='manager_update' AND deleted<>'1'
		";
		try{
	...
	case 'view-campaign-report':
		...
		while($tracker = $rsTracking->fetch(PDO::FETCH_ASSOC)){
			...
			$memberID = $tracker['member_id'];
			
			$hasMemberID = true;
			if(strpos($memberID, '@') !== false){
				$hasMemberID = false;	
				
				$nameFirst 	= $tracker['name_first'];
				$nameLast	= $tracker['name_last'];
				$username	= $tracker['username'];	
			}else{
				
				$memberInfo = get_member($mLink, $memberID, 'all');
				
				$nameFirst 	= $memberInfo['firstName'];
				$nameLast	= $memberInfo['lastName'];
