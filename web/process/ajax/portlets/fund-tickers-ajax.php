...
//	$sSmallName =	(!$compliant ? "<strong style='color:#ed4e2a'>" : "").
					...
					if(!$compliant){
						$fundLabel = '<span class="label label-danger">';
					}elseif($negCash){
						$fundLabel = '<span class="label label-warning">';
					}else{
						$fundLabel = '';	
					}
	
	/*$sName =		(!$compliant || $negCash ? "<strong style='color:#ed4e2a;'>" : "").
//					'<span style="text-decoration:underline;">'.$fund['fund_symbol'].'</span>'.
					$fund['fund_symbol'].
					(!$compliant ? "<sup> &plusmn;</sup>" : "").
					($negCash ? "<sup> !</sup>" : "").
					(!$compliant || $negCash ? "</strong>" : "")
	;*/
	
	$sName =		$fundLabel.$fund['fund_symbol'].
					(!$compliant ? "<sup> &plusmn;</sup>" : "").
					($negCash ? "<sup> -$</sup>" : "").
					(!$compliant || $negCash ? "</span>" : "")
	;
	/*$sCash =		($negCash ? "<strong style='color:#ed4e2a;'>" : "").
					number_format(round($livePrice['cashValue'], 2), 2).
					($negCash ? "</strong>" : "")
	;*/
	$sCash =		($negCash ? '<span class="label label-warning">' : "").
					number_format(round($livePrice['cashValue'], 2), 2).
					($negCash ? "</span>" : "")
	;
	...
	;
	$sBubble =		"Name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$fund['fund_name'].($negCash ? " (NEGATIVE CASH!)" : "")."\n".
					"Type:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".($fund['short_fund'] ? "Short" : "Long")." Fund (".(!$compliant ? "NOT " : "")."Compliant)\n".
					"Inception:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".date("m-d-Y", $fund['unix_date']).
					"\nUpdated:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".date('m/d/Y h:i:s a', $livePrice['timestamp'])
	; // Bubble data (fund type, compliance, inception, description)
?>
			<!-- Build the fund's table row -->
			<?php /*?><tr title="<?php echo $sBubble; ?>" onClick="location.href='?page=03-01-03-001&fund=<?php echo $fund['fund_id']; ?>';" style="cursor:pointer;"><?php */?>
            <tr title="<?php echo $sBubble; ?>">
...
<!--				<td class="hidden-lg"><?php echo $sSmallName; ?></td>-->
				<td onClick="location.href='?page=03-01-03-001&fund=<?php echo $fund['fund_id']; ?>';" style="cursor:pointer;"><span style=""><?php echo $sName; ?></span></td>
...
<!--				<td class="hidden-xs" style="text-align:right"><?php echo number_format(round($livePrice['cashValue'], 2), 2); ?></td>-->
				<td class="hidden-xs" style="text-align:right"><?php echo $sCash; ?></td>
...
if ($nonCompliant > 0){
?>
	<div style="padding:10px 0 0 0;font-size:10px;">
		<span class="label label-danger"><sup>&plusmn;</sup> Non-Compliant Fund</span>
	</div>
<?php
}
?>
...
?>
	<div style="padding:10px 0 0 0;font-size:10px;">
		<span class="label label-warning"><sup>-$</sup> Negative Cash</span>
	</div>
<?php
}
?>
