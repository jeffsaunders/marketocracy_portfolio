...
echo "Trading has halted.";die();


$symbols 	= strtoupper($_REQUEST['symbols']);
$tradeType 	= $_REQUEST['trade'];
$wiz		= $_REQUEST['wiz'];
$urlFund	= $_REQUEST['fund'];

$wizardLink	= '?page=02-00-00-001';
...
switch($wiz){
	...
	case 'buy'	: 
		$wizName		= 'Buy Wizard';
		$portName		= 'Buy Positions';
		$wizNote		= '<p>Trade Wizard is in "BUY" mode. <br /><br />Note: if you are using a smaller screen, some columns might be automatically hidden for better viewing. You can toggle these columns by using the "Show Columns" button above the trades.</p>';
		$tradeLabel		= 'Buy';
		$symbolMessage	= 'Enter stock symbols you would like to BUY for '.$fundSymbol.'.';
		$buyNote		= 'Need to add a symbol? Just enter a symbol in the field provided above, at the end of the trade table, and click on "Add Symbol" to continue. Once the new row appears you can continue the process of adding symbols.';
		
	break;
	case 'sell'	:
		$wizName		= 'Sell Wizard';
		$portName		= 'Sell Positions';
		$wizNote		= '<p>Trade Wizard is in "SELL" mode.<br /><br />Note: if you are using a smaller screen, some columns might be automatically hidden for better viewing. You can toggle these columns by using the "Show Columns" button above the trades.<p>';
		$tradeLabel		= 'Sell';
		$symbolMessage	= 'Enter stock symbols you would like to SELL for '.$fundSymbol.'.';
	break;
	default		:
		$wizName		= 'Trade Wizard';
		$portName		= 'Bulk Trade';
		$wizNote		= '<p>Note: if you are using a smaller screen, some columns might be automatically hidden for better viewing. You can toggle these columns by using the "Show Columns" button above the trades.</p>';
		$tradeLabel		= '';
		$symbolMessage	= 'Enter stock symbols';
	break;
...
<!--            	<div class="note note-warning">
                	<h3><i class="icon-warning-sign"></i> Note To Members!</h3>
                    <p>We are currently experiencing technical difficulties with our Live Price Data Feed provider. This is causing some symbols to not show in the Trade Wizard. Please be patient as we are working this issue as quickly as possible.</p>
                </div>
-->            
               <div class="portlet" id="trade_wizard">
                  <div class="portlet-title">
                     <div class="caption">
                        <i class="icon-random"></i> <?php echo $wizName;?> - <span class="step-title">Step 1 of 4</span>
                     </div>
                           ...
                           <div class="form-body">
                              ...
                              <div class="tab-content">
                                 ...
                                 <div class="tab-pane active" id="tab1">
                                    ...
                                    <h3 class="block"><?php echo $symbolMessage;?></h3>
                                    
                                    <div class="form-group" <?php if($tradeType != ''){echo'style="display:none"';}?>>
                                    ...
                                    <div class="form-group" <?php if($fundID != ''){echo'style="display:none"';}?>>
                                        ...
                                        <div class="col-md-4" id="fund-selection">
                                        	<select class="form-control" name="funds_front" id="funds-front" onchange="getSymbols(); checkFund();">
												...
												$query = "
													...
													WHERE mf.member_id=:member_id AND active='1' AND short_fund='0'
												...
												while($fund = $rsGetFunds->fetch(PDO::FETCH_ASSOC)){
													...
													$cash = number_format($fund['cashValue'],2,'.',',');
													...
													/*
													if($_REQUEST['trade'] == 'sell' && $cash < 0){
														$disableOption = 'disabled';
													}*/
													
													if($cash < 0){
														$cash = '<span style="color:#ED4E2A;">($'.str_replace('-','',$cash).')</span>';	
													}else{
														$cash =	'<span>$'.$cash.'</span>';	
													}
													echo '<option value="'.$fund['fund_id'].'" '.$disableOption.'>'.$fund['fund_symbol'].' : '.$cash.'</option>';
                                    	...
                                    	<div class="col-md-4">
                                    		<div class="checkbox-list" id="show-funds" style="display:none;">
												...
												while($fund = $rsGetFunds->fetch(PDO::FETCH_ASSOC)){
													...
													?>
                                                    <label class="checkbox-inline" style="padding-left:0px;margin-left:0px;" title="Avaliable Cash">
                                                		<input type="checkbox" name="include_funds[]" value="<?php echo $fund['fund_id'];?>" checked> <?php echo $fund['fund_symbol'];?> : <?php echo $cash;?>
                                                	</label>
                                                    <?php
												}
												?>
                                    ...
                                    <div class="form-group">
                                    	<?php if($_REQUEST['buySym'] != ''){?>
                                       		<a href="<?php echo $wizardLink;?>&fund=<?php echo $fundID;?>&trade=buy&wiz=buy&symbols=<?php echo $_REQUEST['buySym'];?>" class="btn btn-info btn-md">Buy Exisiting Positions</a>
                                        <?php } ?>
                                       	<label class="control-label col-md-3">Symbols<span class="required">*</span></label>
                                       	<div class="col-md-4">
                                          	<input type="text" class="form-control" name="symbols" id="symbols" value="<?php echo $symbols;?>" style="text-transform: uppercase !important;" />
                                          	...
                                          	<span class="help-block">Enter multiple stock symbols seperated by a comma(,). <strong>You must enter at least (1)One symbol, you will be able to add additonal symbols on the next screen.</strong></span>
                                       	</div>
                                    </div>
                                 ...
                                 <div class="tab-pane " id="tab2">
                                    ...
                                    <!-- BEGIN TRADE TABLE-->
                                     <div class="portlet">
                                        <div class="portlet-title">
                                           <div class="caption">
                                           	<i class="icon-random"></i> <?php echo $portName;?></div><!--caption-->
                                           <div class="actions">
                                        ...
                                        <div class="portlet-body">
                                        	...
                                        	<table class="table table-striped table-bordered table-hover table-full-width" id="cash-table">
												...
												$query = "
													...
													FROM ".$_SESSION['fund_table']." AS mf
													INNER JOIN ".$_SESSION['fund_liveprice_table']." as lp ON lp.fund_id=mf.fund_id
													WHERE mf.member_id=:member_id AND active='1'
													ORDER BY mf.seq_id ASC
												";
												...
												while($fund2 = $rsGetFunds2->fetch(PDO::FETCH_ASSOC)){
													...
													$query = "
														...
														WHERE fund_id=:fund_id AND action='buy' AND status='pending'
													";
													try{
														$rsGetActiveTrades = $mLink->prepare($query);
														$aValues = array(
															':fund_id'		=> $fundID2
														);
														$preparedQuery = str_replace(array_keys($aValues), array_values($aValues), $query); //Debug
														$rsGetActiveTrades->execute($aValues);
													}
													catch(PDOException $error){
														// Log any error
														file_put_contents($_SESSION['pdo_log'], "-----\rDate: ".date('Y-m-d H:i:s')."\rFile: ". __FILE__ ."\rLine Number: ". __LINE__ ."\rVars:\r".dump_vars(get_defined_vars())."\r", FILE_APPEND);
													...
													while($activeTickets = $rsGetActiveTrades->fetch(PDO::FETCH_ASSOC)){
														
														$cashHold = $cashHold + ($activeTickets['shares']*$activeTickets['quote_price']);
														
													}
													
													#adjust cash value for buys currently pending
													$cashValue = $cashValue - $cashHold;
													...
													$fundCash =  '
														<td>
																...
																<div class="input-icon">
																	...
																	<input type="text" class="form-control" name="'.$fundID2.'-cash" id="'.$fundID2.'-cash" value="'.number_format($cashValue, 2, '.', ',').'" readonly/>
																</div>
																<input type="hidden" id="'.$fundID2.'-calc-cash" name="'.$fundID2.'-calc-cash" value="'.$cashValue.'" />
																
														</td>
													';
													
													//check to see if wizard is in a lock mode, if so only show related fund
													if($wiz != ''){
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu hold-on-click dropdown-checkboxes pull-right">
													<label><input type="checkbox" checked id="type-check" title="type-col" class="col-select" onclick="showHideCol('type-check','type-col');">Trade Type</label>
                                                    ...
                                                    <label><input type="checkbox" checked id="npos-check" title="npos-col" class="col-select" onclick="showHideCol('npos-check','npos-col');">New Size</label>
                                                    <label><input type="checkbox" checked id="buy-check" title="buy-col" class="col-select" onclick="showHideCol('buy-check','buy-col');">Amount$</label>
                              ...
                              <div class="row">
                                 <div class="col-md-12">
                                    ...
                                    <div class="col-md-offset-3 col-md-9" id="change_button">
                                       ...
                                       <a href="javascript:void(0);" onclick="fastExec();" class="btn btn-success" id="button-exec" style="display:none;">Execute Now</a>                        
                                    </div>
                                 </div>
                              </div>
                           </div>
