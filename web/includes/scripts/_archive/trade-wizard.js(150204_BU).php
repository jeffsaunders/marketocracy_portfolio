		...
		if(tradeType == "sell" && fund != "all"){
			...
			$.ajax({
			  ...
			  success: function(data) {
				...
				window.location.replace("?page=02-00-00-001&trade="+tradeType+"&fund="+fund+"&symbols="+symbols);
...
function tradeSymbols(index){
	...
	if(index == "0"){
		...
		$.ajax({
			...
			url: "process/ajax/<?php echo $twProcess;?>?process=2",
			success: function(data){
...
function calculateTotal(cnt, fundID, calcType){
	...
	var tradeType 		= $("input:radio[name='trade-type-"+cnt+"[]']:checked").val();
		...
		$.ajax({
			url: 'process/ajax/<?php echo $twProcess;?>?process=4&price=' + price + '&cash=' + cash + '&tradeType=' + tradeType + '&stock=' + stockSymbol + '&shares=' + shares + '&totalValue=' + totalValue + '&currentShares=' + currentShares + '&fund=' + fundID + '&cnt=' + cnt + '&company=' + company + '&currentPercent=' + currentPercent + '&currentValue=' + currentValue + '&calcType=' + calcType + '&cashPrevious=' + cashPrevious + '&cashAdjust=' + cashAdjust + '&newPercent=' + newPercent + '&newAmmount=' + newAmmount,
