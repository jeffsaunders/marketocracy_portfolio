...
Supporting files:
	...
	JAVASCRIPT	- THIS DOCUMENT JAVASCRIPT includes/scripts/admin-campaigns.js.php
	HTML		- includes/pages/admin-campaigns.php
*/
$processLink	= 'admin-monthly-performance-processes.php';
...
$('form.generate-campaigns-form').on('submit', function() {
	
	
	$('#generate-camp-submit-btn').html('<button type="button" class="btn btn-default">Processing...</button>');
	
	//process php
	$.ajax({
		type: "POST", 
		data: $(".generate-campaigns-form").serialize(),
		url: "process/ajax/<?php echo $processLink;?>?process=create-email-campaign",
		
		success: function(data)
		{
			$('#generate-camp-submit-btn').html('<input type="submit" name="submit-btn" id="generate-camp-btn" value="Generate Campaigns" class="btn btn-success" />');
			//alert(data);
			//$('#sendEmail-errors').html(data);
			if (data.indexOf("alert-danger") !=-1) {
				$('#gcf-form-errors').html(data);
...
function loadSendCampaignForm(){
	$('form.send-campaign-form').on('submit', function() {
		...
		$.ajax({
			...
			url: "process/ajax/admin-campaigns-processes.php?process=send-campaign",
...
function loadEmails(skipTableInit){
	...
	$.ajax({
        ...
        url: "process/ajax/admin-campaigns-processes.php",
		success: function(data){
...
function loadEditEmailForm(){
	...
	$('form.edit-campaign-form').on('submit', function() {
		...
		$.ajax({
			...
			url: "process/ajax/admin-campaigns-processes.php?process=save-email-campaign",
