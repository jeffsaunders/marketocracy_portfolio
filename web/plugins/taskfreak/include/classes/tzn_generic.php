...
class Tzn {
    ...
    function addProperties($arrProps) {
    	if (is_array($arrProps)) {
			if (is_array($this->_properties)) {
				$this->_properties = array_merge($this->_properties,$arrProps);
			} else {
				$this->_properties = $arrProps;
			}
		}
    }
	...
	/**
	 * getDec (key/val,[decimal],[default])
	 */
	...
	function getDtm($keyval,$format='SQL',$tz=TZN_TZDEFAULT,$default='')
	{
		...
		if ($value == 'now' || $value == 'NOW') {
			$value = TZN_NOW;
		} else if ((ereg('^(0000|9999)',$value)) 
			|| (($dt = @strtotime($value)) === -1)) 
		{
			if (!ereg('^[0-9]{4}\-[0-9]{2}\-[0-9]{2}',$default)) {
				return $default;
			} else {
				$value = $default;
			}
	...
	function setAuto(&$data, $nested = '', $method='_setFromHttp')
	{
        ...
        foreach($this->_properties as $key => $type) {
        	...
        	} else if ((eregi('^IMG',$type)) && ($method == '_setFromHttp')) {
        		// upload
    ...
    function _setFromHttp(&$data,$key,$dkey) {
    	if ($value = $data[$dkey]) {
			if (get_magic_quotes_gpc() || @constant('TZN_MAGIC_FIX')) {
				// remove bloody magic quotes
	...
	{
		$value = ($value)?trim($value):$value;
		if ($value && (!eregi("^(http|https|ftp)?://",$value))) {
			$value = "http://".$value;
		}
		if ($value == "http://") {
			$value = "";
	...
	function uplImg($key)
	{
		...
		if ($ok) {
			...
			if (file_exists(TZN_FILE_TEMP_PATH.$value->tempName)) {
				$this->$key = $value;
				return true;
			} else {
		...
		,$style='tznFormDate',$xtra='')
	{
		...
		$form = ', <input type="text" name="'.$name.'Time" value="'
			.substr($value,11,5).'" ';
		if (is_int($style)) {
			$form .= 'size="'.$style.'" ';
		} else {
    ...
    {
    	if (@constant('TZN_TRANS_ID')) {
			if (session_id() && (!ereg(session_id(),$url))) {
