...
class Item extends TznDb
{
    ...
    function getContext($mode=false) { // mode=0/short, 1/long
		...
		if ($mode) {
            ...
            return '<span style="background-color:'.$GLOBALS['confContext'][$this->context].'" title="'.$str.'">'.substr($str,0,1).'</span>';
        }
	}
    ...
    function getDeadline($short=false) {
        if (ereg('(9999|0000)',$this->deadlineDate)) {
			...
			if ($diff < 0) {
                ...
                $diff = $diff / 3600 / 24;
				if (@constant('FRK_DATEDIFF_MODE') =='day') {
